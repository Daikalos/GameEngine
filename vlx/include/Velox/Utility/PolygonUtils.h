...
namespace vlx::py
{
	...
	static float ComputePolygonArea(std::span<const Vector2f> vertices)
	{
		float total_area = 0.0f;
	...
	static bool IsPointInTriangle(const Vector2f& p, const Vector2f& a, const Vector2f& b, const Vector2f& c)
	{
		Vector2f ab = Vector2f::Direction(a, b);
	...
	static std::optional<std::vector<uint32>> Triangulate(std::span<const Vector2f> vertices)
	{
