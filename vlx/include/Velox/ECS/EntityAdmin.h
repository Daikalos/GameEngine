...
namespace vlx
{
	...
	inline C* EntityAdmin::GetComponent(const EntityID entity_id)
	{
		...
		for (std::size_t i = 0; i < archetype_id.size(); ++i)
		{
			...
			if (id == component_id)
			{
				C* components = static_cast<C*>(&archetype->component_data[i][0]);
	...
	inline void EntityAdmin::RemoveComponent(const EntityID entity_id)
	{
		...
		if (std::find(old_archetype->type.begin(), old_archetype->type.end(), comp_type_id) == old_archetype->type.end())
			return;
